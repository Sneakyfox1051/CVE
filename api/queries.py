def get_vulnerabilities_by_cve_id(cve_id, limit=10):
    return f"""
    SELECT * FROM cve_entries
    WHERE cve_id = %s
    LIMIT {limit};
    """
def get_total_vulnerabilities_count():
    return """
    SELECT COUNT(*) AS total FROM cve_entries;
    """
def get_vulnerabilities_by_product_id(product_id, limit=10):
    return f"""
    SELECT cve_entries.*
    FROM cve_entries
...
def get_all_vulnerabilities(page=1, limit=10, rows=10):
    if rows not in [10, 50, 100]:
        raise ValueError("Invalid number of rows. Please select from 10, 50, or 100.")
...
# New function to get metrics for a specific ID
